...
export const changePassword = (oldPassword, newPassword, newPasswordConfirm, accessToken, idToken, userId) => {
  return dispatch => {
...
export const resetPassword = (userId, password, passwordConfirm, accessToken) => {
  return dispatch => {
    if (password !== passwordConfirm) 
      ...
      .then(res => {
        if (res.status === 200)
          dispatch({type: constants.PASSWORD_RESET_COMPLETED, message: 'Password reset successfully. Please use your new password for login'});
